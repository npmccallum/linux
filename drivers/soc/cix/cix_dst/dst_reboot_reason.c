...
static uint32_t g_last_hw_reboot_reason;

unsigned int get_hw_reboot_reason(void)
{
	return g_last_hw_reboot_reason;
}
EXPORT_SYMBOL(get_hw_reboot_reason);
...
static void set_reboot_reason_sky1(unsigned int reboot_reason)
{
	...
	arm_smccc_smc(CIX_SIP_SVC_SET_REBOOT_REASON, reboot_reason, 0, 0, 0, 0,
		      0, 0, &res);
	/*
	 * Ensure SMC write completes before proceeding.
	 * The SMC call may not have fully committed the reboot reason
	 * to retention memory by the time we return.
	 */
	mb();
	udelay(100);
}
...
void plat_pm_system_reset_comm(const char *cmd)
{
	...
	if (curr_reboot_type == UNKNOWN) {
		...
		DST_PN("no specific reboot type, using COLDBOOT\n");
	}
...
static void reboot_reason_addr_init(void)
{
	...
	if (hw_reboot_reason_addr) {
		g_last_hw_reboot_reason =
			readl((void *)(uintptr_t)hw_reboot_reason_addr);
		print_hw_reboot_reason(g_last_hw_reboot_reason);
	}
}
